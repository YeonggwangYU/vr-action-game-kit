...
namespace _3DGamekitLite.Scripts.Editor.Game.Player
{
    [CustomEditor(typeof(CheckpointGroup))]
    public class CheckpointGroupEditor : UnityEditor.Editor
    {
        public void OnSceneGUI()
        {
            ...
            if (Event.current.control && Event.current.type == EventType.MouseDown && Event.current.button == 0)
            {
                ...
                var ray = UnityEngine.Camera.current.ScreenPointToRay(sp);
                RaycastHit hit;
